module "linux_web_app" {
  ...
  app_ssl_cert_name     = module.get_ssl_certificate["marketagora-ssl"].name
  ssl_cert_kv_secret_id = module.get_ssl_certificate["marketagora-ssl"].secret_id

  depends_on = [
    module.resource_group["app"].id,
    module.service_plan["asp"].id,
    module.get_ssl_certificate["marketagora-ssl"].secret_id
  ]
}
